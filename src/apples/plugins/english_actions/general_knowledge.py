...
@storage.misc_dumper(datetime, "datetime", 1)
...
@storage.misc_loader("datetime", 1)
def _datetime_loader(data, _):
    return datetime(
        year=data["year"],
        month=data["month"],
        day=data["day"],
...
@storage.misc_dumper(dateutil.tz.tzutc, "tzutc", 1)
def _tzutc_dumper(_):
    return {}


@storage.misc_loader("tzutc", 1)
def _tzutc_loader(_, __):
    return dateutil.tz.tzutc()


@storage.misc_dumper(dateutil.tz.tzlocal, "tzlocal", 1)
def _tzlocal_dumper(_):
    return {"_": 0}


@storage.misc_loader("tzlocal", 1)
def _tzlocal_loader(_, __):
...
class DateTime(Thing):
    ...
    def from_datetime_string(cls, datetime_str, timezone=_local,
                             formatting='%m/%d/%Y %H:%M:%S', **kwargs):
        time = datetime.strptime(datetime_str, formatting)
...
@storage.misc_dumper(Relation, "Relation", 1)
...
@storage.misc_loader("Relation", 1)
def _relation_loader(data, _):
...
class Person(Thing):
    ...
    def update_pronouns(self):
        self.session.put(ThingReference(self.name, self.session), "who")
        if self.gender == "Male":
            self.session.put(ThingReference(self.name, self.session), "he")
        elif self.gender == "Female":
            self.session.put(ThingReference(self.name, self.session), "she")
        else:
    ...
    def _like_action(self, verb):
        ...
        if "do" in verb.aux or verb.punct == "?":
            ...
            for obj in verb.obj:
                ot = obj.thing
                # if ot in self.opinions:  # I used to use the objects as reference, the serializer does not rebuild
                # those objects properly. When I can get it to, this will probably change back.
...
# Create objects.
with storage.get_default_storage().get_session() as session:
    with open(__file__[:-20] + "color-names/color-names.json") as color_file:
        ...
        for hex_value, name in color_list.items():
            name = name.lower()
            general_knowledge[name] = Color(name=name, color=hex_value, session=session)
    ...
    _holo = general_knowledge["holo"] = SystemPersonality(
        ...
        opinions=[(Thing.get_by_name("apple", session), 10)],  # Apples are a hint
        birthday=Thing.get_by_name("holo.birthday", session),
        subjective="i",
        objective="me",
        possessive="my",
        tense="",
    ...
    _prism = general_knowledge["prism"] = SystemPersonality(
        ...
        opinions=[(Thing.get_by_name("light", session), 10)],
        birthday=Thing.get_by_name("prism.birthday", session),
        subjective="i",
        objective="me",
        possessive="my",
        tense="",
    ...
    _holo.relations.append(Relation(Thing.get_by_name("prism", session), "sister"))
    _prism.relations.append(Relation(Thing.get_by_name("holo", session), "sister"))
    # noinspection PyTypeChecker
    ...
    general_knowledge["you"] = ThingReference("holo", session)
    session.put(general_knowledge["you"], "you")
    general_knowledge["people"] = ThingReference("person", session)
    session.put(general_knowledge["people"], "people")
