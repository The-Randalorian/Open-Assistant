...
al_paras = {
    '572295c965377c5f73b6aeb5f7dbf6c8383cc2c6c2aaf4f5889bdefe7d32e26905b88c9e6e12a66b4d644a3ca27ca88df1a2541e0c28a515c3'
    '432a357550a380',
    'f13c207045bb82fac768f1536c3565d34883435acac89645992dfa4775be9285cdd7ac62ad8a5cd744f4617d5bffb88764dd37463b03fee48c'
    '9b34e6fa844353',
    '9f4ecffb7d1f04514c687040d607d03a2f9803f9da367b7e7b73480a65138e19ca63ba297d4d3d612dd9c90d8ff6a5fe9246c0d1d3dec73bc7'
    'c08809d3ec557e'
}
...
class Noun(Word):
    def __init__(self, session: storage.ThingStorageSession, **kwargs):
        t = kwargs.get("text")
        self.session = session
        self.thing = kwargs.pop("thing", understanding.Thing.get_by_name(name=t, session=self.session))
    ...
    @classmethod
    def get_by_name(cls, name, session: storage.ThingStorageSession, called=None, **kwargs):
        t = understanding.Thing.get_by_name(name, session)
        if called is not None:
            t.objective = getattr(t, "objective", called)
        return cls(thing=t, text=name, session=session, **kwargs)

    @classmethod
    def get_from_stanza(cls, sentence, word, session: storage.ThingStorageSession, **kwargs):
...
def get_system_noun(session):
    return Noun.get_by_name("holo", session)
...
class Verb(Word):
    ...
    def __init__(self, session: storage.ThingStorageSession, **kwargs):
        self.session = session
        self.subj = kwargs.pop("subj", [get_system_noun(self.session)])
    ...
    @classmethod
    def get_from_stanza(cls, sentence, word, session: storage.ThingStorageSession, base_subj: typing.Sequence, **kwargs):
        ...
        if len(subjects) <= 0:
            ...
            nsubjs = []
            for ob in subjects:
                nsubjs.append(Noun.get_from_stanza(sentence, ob, session))
        obls = []
        for ob in get_dep_and_conj_flat(sentence, word, "obl"):
            obls.append(Noun.get_from_stanza(sentence, ob, session))
        objs = []
        for ob in get_dep_and_conj_flat(sentence, word, "obj"):
            objs.append(Noun.get_from_stanza(sentence, ob, session))
        iobjs = []
        for ob in get_dep_and_conj_flat(sentence, word, "iobj"):
            iobjs.append(Noun.get_from_stanza(sentence, ob, session))
        advmods = []
...
def run_root(sentence, root, session, base_subjs, default_subjects=None):
    if not default_subjects:
        default_subjects = [get_system_noun(session)]
    ...
    for root_verb in get_conjuncts(sentence, root):
        v = Verb.get_from_stanza(sentence, root_verb, session, base_subjs)
        for bandaid in post_bandaids:
            bandaid(sentence, v, session)
        _logger.debug("Processing Verb %s:\n%s", v.text, v)
        for subj in v.subj:
            yield subj.thing.perform_action(v)


def ask_is_one(sentence, root, session):
    nsubjs = get_dep_and_conj_flat(sentence, root, "nsubj")
    nsubjs = [Noun.get_from_stanza(sentence, nsubj, session) for nsubj in nsubjs]
    for classification in get_conjuncts(sentence, root):
        class_noun = Noun.get_from_stanza(sentence, classification, session)
        for nsubj in nsubjs:
            try:
...
def process_text(text: str, storage_context: typing.Optional[storage.ThingStorage] = None):
    if storage_context is None:
        storage_context = storage.get_default_storage()
    with storage_context.get_session() as session:
        ...
        doc = nlp(text)
        root = None
        base_subjs = [get_system_noun(session)]
        ...
        for sentence in doc.sentences:
            for token in sentence.words:
                ...
                if token.deprel == "root":
                    root = token
                    if token.upos == "VERB":
                        yield from run_root(sentence, root, session, base_subjs)
                        ...
                        if base_subj == ():
                            base_subj = [get_system_noun(session)]
                        base_subjs = base_subj
                        ...
                        for root in get_conjuncts(sentence, root):
                            ...
                            for copula in get_dep_and_conj_flat(sentence, root, "cop"):
                                if copula.id < base_subj[0].id:
                                    yield from ask_is_one(sentence, root, session)
                                    asking = True
                            if asking:
                                continue
                            ...
                            if asking:
                                continue
                            base_subj = [Noun.get_from_stanza(sentence, nsubj, session) for nsubj in base_subj]
                            ...
                            for clause in itertools.chain(
                                ...
                                if clause.upos == "VERB":
                                    yield from run_root(sentence, clause, session, base_subjs, default_subjects=base_subj)
            session.commit()
...
def bandaid_turn_on_obl(sentence, verb, session):
...
def bandaid_turn_on_nmod(sentence, verb, session):
    if verb.text in ("turn", "switch", "toggle") \
        ...
        for word in sentence.words:
            if word.lemma in ("on", "off") \
                ...
                verb.obj.append(Noun.get_from_stanza(sentence, word, session))


def bandaid_turn_on_weird(sentence, verb, session):
    if verb.text in ("turn", "switch", "toggle") \
...
if __name__ == "__main__":
    ...
    data = storage.dump(understanding.Thing.get_by_name("holo").get_ref())
    print(len(data))
    print(data.hex())
    print("\n" + "=" * 60 + "\n")
